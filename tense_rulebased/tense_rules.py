...
def passive_VBG_child(predicate,passive_child):
    ...
    flag_VBP_VBZ,span_VBP_VBZ = predicate._get_span_of_filtered_children(VBP_or_VBZ_child_func)
    flag_VBD_VBN,span_VBD_VBN = predicate._get_span_of_filtered_children(VBD_or_VBN_child_func)
...
def passive_VB_child(predicate,passive_child):
    flag_TO,span_TO = predicate._get_span_of_filtered_children(TO_child_func)
    flag_FUTURE_MD,span_FUTURE_MD = predicate._get_span_of_filtered_children(FUTURE_MD_child_func)
    ...
    if predicate.pos == VBN and passive_child.pos == VB and passive_child.word == BE:
        ...
        if  flag_TO :
            if predicate.get_parent_relation() in clausal_complement and (predicate.parent.word.lower() == "going" or predicate.parent.word.lower() == "about"):
...
def infinitives_future(predicate):
    ...
    if flag_FUTURE_MD:
        return (TENSE_FUTURE, "to")


# past - "they had already eaten dinner"
def past_VBN_pos(predicate):
    flag_VBD,span_VBD = predicate._get_span_of_filtered_children(VBD_child_func)
...
def past_VBG_pos(predicate):
    flag_VBD,span_VBD = predicate._get_span_of_filtered_children(VBD_child_func)
    flag_VBN,span_VBN = predicate._get_span_of_filtered_children(VBN_child_func)
...
def present_VBN_pos(predicate):
    flag_VBP_or_VBZ,span_VBP_or_VBZ = predicate._get_span_of_filtered_children(VBP_or_VBZ_child_func)
...
def future_VB_pos(predicate):
    ...
    if predicate.pos == VB and flag_TO and (predicate.get_parent_relation() in clausal_complement) and (predicate.parent.word.lower() == "going" or predicate.parent.word.lower() == "about"):
...
def future_VBN_pos(predicate):
    flag_FUTURE_MD,span_FUTURE_MD = predicate._get_span_of_filtered_children(FUTURE_MD_child_func)
    flag_VB_have,span_VB_have = predicate._get_span_of_filtered_children(VB_have_child_func)
    min_FUTURE_MD, max_FUTURE_MD = span_FUTURE_MD
...
def future_VBG_pos(predicate):
    ...
    min_FUTURE_MD, max_FUTURE_MD = span_FUTURE_MD
    flag_VB_have,span_VB_have = predicate._get_span_of_filtered_children(VB_have_child_func)
    flag_VB_be,span_VB_be = predicate._get_span_of_filtered_children(VB_be_child_func)
    min_VB_be, max_VB_be = span_VB_be
    flag_VBN_been,span_VBN_been = predicate._get_span_of_filtered_children(VBN_been_child_func)
...
#def future_VB_pos_going_to(predicate):
#    flag_TO,span_TO = predicate._get_span_of_filtered_children(TO_child_func)
#    if predicate.pos == VB and flag_TO and predicate.get_parent_relation() in clausal_complement and (predicate.parent.word.lower() == "going" or predicate.parent.word.lower() == "about"):
#        return (TENSE_FUTURE,True)
...
def get_tense(predicate,run_simple_rules = True):
    ...
    elif flag_TO  and predicate.get_parent_relation() in clausal_complement and predicate.parent.word.lower() != "going" and predicate.parent.word.lower() != "about":
